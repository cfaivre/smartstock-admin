var ItemArrivalHelper = function (data, config) {
  'use strict';
  var  _viewModel,
       _applyKoBindings,
       _currentContext,
  ...
  _applyKoBindings = function () {
    ...
    _viewModel.readArrivalStock = function() {
      _viewModel.new_storage_location = null;
      _viewModel.isLoading = ko.observable( true );
      $( "#btnReadStock" ).val('Scan again');
      var jsdata = _mapToJSON( _viewModel.rfids() );
      core.infrastructure.ajaxPost( '/item/read_arrived_items', jsdata, null,
                                   _currentContext,
                                   _itemsCallback,
                                     null );
    };
