class ItemDepartureModel
  ...
  def load
    viewmodel = ItemDepartureViewModel.new
    viewmodel.items = []
    viewmodel.user = @username
#    items = StockApiClient.new.get_items
#    items.sort_by! { |item| item["name"] } unless items.empty?
    viewmodel.current_location = @username
    viewmodel
  end

  def read_departure_items( already_read_rfids )
  ...
  def read_items( already_read_rfids )
    ...
    items = [] # TODO: Add it to a list and display it in the frontend
